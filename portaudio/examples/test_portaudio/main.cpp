...
int main(int argc, char **argv)
{
    ...
    {
        for(int i = 0; i < ports.size(); std::cout << ports[ i++ ] << endl)
        {
        };
        return 0;
    }
    ...
    while(true)
    {
        ...
        if(device->Writev((char*)line.c_str(), line.size(), timeout) != line.size())
        {
        ...
        do
        {
            ...
            readed = device->Readv(receiveBuf, sizeof(receiveBuf) - 1, timeout);
